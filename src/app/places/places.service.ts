...
export class PlacesService {
    ...
    return this.http.get<{ [key: string]: PlaceData }>('https://ionic-angular-bookings-531f1.firebaseio.com/offered-places.json').pipe(map(data => {
      ...
      for (const key in data) {
        if (data.hasOwnProperty(key)) {
          places.push(new Place(key, data[key].title, data[key].description, data[key].imageUrl, data[key].price, new Date(data[key].availableFrom), new Date(data[key].availableFrom), data[key].userId));
        }
      }
      return places;
    }),
  ...
  getPlace(id: string) {
    return this.http.get<PlaceData>(`https://ionic-angular-bookings-531f1.firebaseio.com/offered-places/${id}.json`).pipe(map(data => {
      return new Place(id, data.title, data.description, data.imageUrl, data.price, new Date(data.availableFrom), new Date(data.availableTo), data.userId);
    }));
  }
